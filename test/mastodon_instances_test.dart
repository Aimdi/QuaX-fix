...
void main() {
  group('mastodonInstanceCandidates', () {
    test('asks the account\'s own instance first — it is the only complete one', () {
      final candidates = mastodonInstanceCandidates('reader@example.social');

      expect(candidates.first, 'https://example.social');
      expect(candidates.skip(1), kMastodonDefaultInstances, reason: 'defaults follow, in their stated order');
    });

    test('the reader\'s instances come between the origin and the defaults', () {
      final candidates = mastodonInstanceCandidates(
        'reader@example.social',
        configured: ['https://my.home', 'second.place'],
      );

      expect(candidates.take(3), ['https://example.social', 'https://my.home', 'https://second.place']);
    });

    test('a bare local name has no origin, so the reader\'s home leads', () {
      final candidates = mastodonInstanceCandidates('reader', configured: ['https://my.home']);

      expect(candidates.first, 'https://my.home');
      expect(candidates, isNot(contains(startsWith('https://reader'))));
    });

    test('never asks the same instance twice, however many ways it was named', () {
      final candidates = mastodonInstanceCandidates(
        'reader@mastodon.social',
        configured: ['mastodon.social', 'https://mastodon.social/'],
      );

      expect(candidates.where((e) => e == 'https://mastodon.social'), hasLength(1));
      expect(candidates.first, 'https://mastodon.social');
    });

    test('with nothing configured there is still a full list — that is the point', () {
    ...
    test('junk in the configured list is dropped rather than asked', () {
      final candidates = mastodonInstanceCandidates('reader', configured: ['not a host', '']);
  ...
  group('firstInstanceThat', () {
    test('a miss on one instance moves to the next, and the winner\'s answer is returned', () async {
      final asked = <String>[];
    ...
    test('when every instance fails, the most telling failure is the one reported', () async {
      final client = MastodonClient(
        httpClient: MockClient((request) async {
      ...
      await expectLater(
        client.lookupAnywhere(['https://a.place', 'https://b.place'], 'reader'),
    ...
    test('an empty candidate list is not configured, not a crash', () async {
      final client = MastodonClient(httpClient: MockClient((_) async => _json(const [])));

      await expectLater(
        client.lookupAnywhere(const [], 'reader'),
        throwsA(isA<MastodonException>().having((e) => e.kind, 'kind', MastodonErrorKind.notConfigured)),
      );
    });
  });
    ...
    test('home first, then the added list, in order', () {
      final prefs = PrefServiceCache(
        cache: {
          ...
          optionPluginMastodonInstances: jsonEncode(['https://a.place', 'https://b.place']),
        },
      );

      expect(mastodonConfiguredInstances(prefs), ['https://my.home', 'https://a.place', 'https://b.place']);
    });

    test('nothing configured is an empty list, and corrupt storage reads the same', () {
      expect(mastodonConfiguredInstances(PrefServiceCache(cache: {})), isEmpty);
      expect(
        mastodonConfiguredInstances(PrefServiceCache(cache: {optionPluginMastodonInstances: 'not json'})),
        isEmpty,
      );
    });
  });
