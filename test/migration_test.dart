...
void main() {
  ...
  test('fresh onCreate reaches databaseVersion with core tables', () async {
    ...
    expect(
      accountCols,
      containsAll(['id', 'auth_header', 'screen_name', 'last_not_found_at', 'consecutive_not_found']),
    );
  });
  ...
  test('upgrade from v22 preserves account auth_header and subscription rows', () async {
    ...
    final subscription =
        UserSubscription.fromMap((await db.query(tableSubscription, where: 'id = ?', whereArgs: ['783214'])).single);
