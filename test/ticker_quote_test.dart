...
void main() {
  group('reading a price history', () {
    ...
    test('integers and numeric strings are both prices', () {
      final quote = TickerQuote.fromChartJson(_chart(closes: [100, '110.5', 120.0]), symbol: 'AAPL')!;

      expect(quote.points.map((p) => p.close), [100.0, 110.5, 120.0]);
    });

    test('mismatched list lengths stop at the shorter one', () {
      final quote = TickerQuote.fromChartJson(_chart(timestamps: [1, 2, 3], closes: [1.0]), symbol: 'AAPL')!;
  ...
  group('the day\'s move', () {
    test('is measured against the previous close', () {
      final quote = TickerQuote.fromChartJson(
          _chart(meta: {'symbol': 'AAPL', 'regularMarketPrice': 110.0, 'chartPreviousClose': 100.0}),
          symbol: 'AAPL')!;
    ...
    test('falls back to the last close when no live price is quoted', () {
      final quote = TickerQuote.fromChartJson(
          _chart(closes: [100.0, 110.0, 130.0], meta: {'chartPreviousClose': 100.0}),
          symbol: 'AAPL')!;
  ...
  group('a payload that no longer fits', () {
    test('gives up rather than throwing', () {
      for (final json in <Object?>[
        ...
        expect(TickerQuote.fromChartJson(json, symbol: 'AAPL'), isNull, reason: '$json');
      }
    });

    test('history with no usable closes is nothing to draw', () {
      expect(TickerQuote.fromChartJson(_chart(closes: [null, null, null]), symbol: 'AAPL'), isNull);
    });

    test('the symbol asked for is used when the response omits its own', () {
      final quote = TickerQuote.fromChartJson(_chart(meta: const {}), symbol: 'msft')!;
  ...
  group('the client', () {
    ...
    test('an unknown symbol is reported as not found', () async {
      final client = TickerClient(httpClient: MockClient((_) async => http.Response('{}', 404)));
    ...
    test('a refusal is unavailable, not a broken response', () async {
      final client = TickerClient(httpClient: MockClient((_) async => http.Response('', 429)));
    ...
    test('a 200 that is not JSON is a bad response', () async {
      final client = TickerClient(httpClient: MockClient((_) async => http.Response('<html>', 200)));

      await expectLater(
        client.fetchQuote('AAPL'),
        throwsA(isA<TickerException>().having((e) => e.kind, 'kind', TickerErrorKind.badResponse)),
      );
    });

    test('a good response becomes a quote, and carries an agent', () async {
      late http.Request sent;
      final client = TickerClient(httpClient: MockClient((request) async {
        sent = request;
