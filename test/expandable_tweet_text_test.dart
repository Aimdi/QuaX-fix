...
void main() {
  testWidgets('a post that fits shows no expand link', (tester) async {
    await tester.pumpWidget(_wrap(
      const ExpandableTweetText(textSpans: [TextSpan(text: 'Short one.')], maxLines: 8),
    ));
  ...
  testWidgets('a clipped post offers the link and expands on tap', (tester) async {
    await tester.pumpWidget(_wrap(
      const ExpandableTweetText(textSpans: [TextSpan(text: _long)], maxLines: 3),
    ));
    await tester.pumpAndSettle();
  ...
  testWidgets('lines are counted at the painted width, not the screen width', (tester) async {
    ...
    await tester.pumpWidget(_wrap(
      const ExpandableTweetText(textSpans: [TextSpan(text: 'Short one.')], maxLines: 2),
      width: 780,
    ));
    await tester.pumpAndSettle();
  ...
  testWidgets('maxLines null never clips or offers the link', (tester) async {
    await tester.pumpWidget(_wrap(
      const ExpandableTweetText(textSpans: [TextSpan(text: _long)], maxLines: null),
    ));
    await tester.pumpAndSettle();
