...
///
/// A chart alone answers "which way" and nothing else. These are the three
/// numbers every market page puts under one, and both places that draw a price
/// — the ticker screen and the watchlist — want the same three.
class TickerStats extends StatelessWidget {
  ...
  static final NumberFormat _price = NumberFormat.decimalPatternDigits(decimalDigits: 2);
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      children: [
        _stat(theme, l10n.plugin_stocks_volume, quote.volume, format: _volume.format),
        ...
        if (showCurrency && currency != null)
          Expanded(
            child: Align(
              ...
              child: Text(currency, style: theme.textTheme.labelSmall),
            ),
          ),
      ],
    );
  ...
  Widget _stat(ThemeData theme, String label, double? value, {String Function(double)? format}) {
    return Padding(
