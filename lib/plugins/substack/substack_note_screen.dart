    ...
    return Scaffold(
        ...
        actions: [
          if (note.url != null) ...[
            IconButton(
              ...
              onPressed: () => SharePlus.instance.share(ShareParams(text: note.url!)),
            ),
            IconButton(
      ...
      body: ListView(
        ...
        children: [
          Row(
            children: [
              if (note.authorPhotoUrl != null)
                ClipOval(
                  child: ExtendedImage.network(note.authorPhotoUrl!, width: 40, height: 40, fit: BoxFit.cover),
                )
              else
                CircleAvatar(
                  radius: 20,
                  child: Text(
                    (note.authorName ?? note.authorHandle ?? '?').trim().isEmpty
                        ? '?'
                        : (note.authorName ?? note.authorHandle)!.trim()[0].toUpperCase(),
                  ),
                ),
          ...
          Text(note.body, style: theme.textTheme.bodyLarge!.copyWith(height: 1.45)),
          ...
          if ((note.reactionCount ?? 0) > 0) ...[
            ...
            Row(
              children: [
                Icon(Icons.favorite_outline, size: 16, color: theme.colorScheme.onSurfaceVariant),
          ...
          if (pub != null) ...[
            ...
            Text(l10n.plugin_substack_publication, style: theme.textTheme.labelLarge),
