/// Domains supported by Bypass Paywalls Clean (BPC) 4.4.0.0.
///
/// Extracted from the extension's sites.js / sites_custom.json /
/// sites_updated.json. QuaX cannot run BPC's content scripts or
/// declarativeNetRequest rules; this list only decides which outbound
/// links the plugin claims so they can be opened through a mobile
/// bypass strategy (archive / 12ft / googlebot WebView).
///
/// Source: Bypass Paywalls Clean, MIT License, © magnolia1234.
const Set<String> bpcSupportedDomains = {
  '21news.be',
  '24heures.ch',
  '360dx.com',
  '404media.co',
  '60millions-mag.com',
  'aachener-zeitung.de',
  'abajournal.com',
  'abc.es',
  'abcmais.com',
  'abendblatt.de',
  'abendzeitung-muenchen.de',
  'abqjournal.com',
  'abril.com.br',
  'acadienouvelle.com',
  'actu-environnement.com',
  'actu.fr',
  'ad.nl',
  'adage.com',
  'adelaidenow.com.au',
  'adformatie.nl',
  'adweek.com',
  'aefinfo.fr',
  'aerokurier.de',
  'aerztezeitung.de',
  'afr.com',
  'aftenposten.no',
  'aftermath.site',
  'aftonbladet.se',
  'airmail.news',
  'aisnenouvelle.fr',
  'ajc.com',
  'al-monitor.com',
  'al.com',
  'algonquinbeyond.com',
  'allgaeuer-zeitung.de',
  'allgemeine-zeitung.de',
  'alternatives-economiques.fr',
  'ambito.com',
  'americanaffairsjournal.org',
  'americanbanker.com',
  'americanpurpose.com',
  'americastestkitchen.com',
  'amplitude.com',
  'anandabazar.com',
  'aoc.media',
  'apollo-magazine.com',
  'ara.cat',
  'arabalears.cat',
  'arabianbusiness.com',
  'architecturaldigest.com',
  'arcinfo.ch',
  'argaam.com',
  'artforum.com',
  'artnet.com',
  'artsprofessional.co.uk',
  'asia.nikkei.com',
  'atavist.com',
  'athensreviewofbooks.com',
  'atlantico.fr',
  'augsburger-allgemeine.de',
  'auto-motor-und-sport.de',
  'auto-swiat.pl',
  'autocar.co.uk',
  'autohebdo.fr',
  'automobilwoche.de',
  'autonews.com',
  'autoplus.fr',
  'autosport.com',
  'axios.com',
  'azcentral.com',
  'backpacker.com',
  'balkaninsight.com',
  'baltimoresun.com',
  'barandbench.com',
  'barrons.com',
  'baseballamerica.com',
  'bazonline.ch',
  'bbc.com',
  'bd.nl',
  'beauxarts.com',
  'beleggersbelangen.nl',
  'bendigoadvertiser.com.au',
  'benzinga.com',
  'beobachter.ch',
  'berkshireeagle.com',
  'berliner-zeitung.de',
  'berlingske.dk',
  'bernerzeitung.ch',
  'betamtb.com',
  'betternutrition.com',
  'betterprogramming.pub',
  'bhaskar.com',
  'bicycling.com',
  'bienpublic.com',
  'bild.de',
  'billboard.com',
  'bizjournals.com',
  'bizwest.com',
  'blick.ch',
  'bloomberg.com',
  'bloombergadria.com',
  'blueconic.net',
  'bm-online.de',
  'bnd.com',
  'bndestem.nl',
  'bnn.de',
  'boersen-zeitung.de',
  'bonappetit.com',
  'bonasavoir.ch',
  'booksandpublishing.com.au',
  'bordermail.com.au',
  'boredpanda.com',
  'boston.com',
  'bostonglobe.com',
  'bostonherald.com',
  'bostonmagazine.com',
  'bote.ch',
  'brainly.com',
  'brandonsun.com',
  'braunschweiger-zeitung.de',
  'brisbanetimes.com.au',
  'britannica.com',
  'brusselstimes.com',
  'bt.no',
  'btc-echo.de',
  'buffalonews.com',
  'bulletinquotidien.fr',
  'business-standard.com',
  'businessam.be',
  'businessdailyafrica.com',
  'businessday.co.za',
  'businessdesk.co.nz',
  'businessinsider.com',
  'businessinsider.com.pl',
  'businessinsider.de',
  'businessinsider.jp',
  'businessinsider.nl',
  'businesslive.co.za',
  'businessoffashion.com',
  'businesspost.ie',
  'businesstimes.com.sg',
  'businesstoday.in',
  'cafe.com',
  'cairnspost.com.au',
  'calgaryherald.com',
  'cambiocolombia.com',
  'canarias7.es',
  'canberratimes.com.au',
  'cannabiz.com.au',
  'capital.bg',
  'capital.fr',
  'capitalaberto.com.br',
  'capitalgazette.com',
  'caravanmagazine.in',
  'cardiologie-pratique.com',
  'cartacapital.com.br',
  'catholicherald.co.uk',
  'causeur.fr',
  'cdn.ampproject.org',
  'cen.acs.org',
  'centralwesterndaily.com.au',
  'centrepresseaveyron.fr',
  'challenges.fr',
  'charentelibre.fr',
  'charliehebdo.fr',
  'charlotteobserver.com',
  'chicago.suntimes.com',
  'chicagobusiness.com',
  'chicagotribune.com',
  'christianitytoday.com',
  'chronicle.com',
  'chunichi.co.jp',
  'churchtimes.co.uk',
  'cicero.de',
  'cincinnati.com',
  'citywire.com',
  'clareecho.ie',
  'clarin.com',
  'cleaneatingmag.com',
  'cleveland.com',
  'climatechangenews.com',
  'climbing.com',
  'cnbc.com',
  'cnn.com',
  'cntraveler.com',
  'cnv-medien.de',
  'codesports.com.au',
  'columbian.com',
  'commentary.org',
  'commercialappeal.com',
  'commercialobserver.com',
  'computerweekly.com',
  'connaissancedesarts.com',
  'connexionfrance.com',
  'cornwallreports.co.uk',
  'correiodopovo.com.br',
  'corren.se',
  'corriere.it',
  'corriereadriatico.it',
  'corrieredellosport.it',
  'cosmopolitan.com',
  'cosmopolitan.fr',
  'countryliving.com',
  'courant.com',
  'courier-journal.com',
  'couriermail.com.au',
  'courrier-picard.fr',
  'courrierinternational.com',
  'craftscouncil.org.uk',
  'crainscleveland.com',
  'crainsdetroit.com',
  'crainsgrandrapids.com',
  'crainsnewyork.com',
  'crikey.com.au',
  'cronista.com',
  'crusoe.com.br',
  'csmonitor.com',
  'ctinsider.com',
  'ctpost.com',
  'curbed.com',
  'cw.com.tw',
  'cxense.com',
  'cyclingnews.com',
  'dagen.no',
  'dagensmedicin.se',
  'dagsavisen.no',
  'dailyadvertiser.com.au',
  'dailyherald.com',
  'dailyliberal.com.au',
  'dailymail.com',
  'dailypress.com',
  'dailyrecord.co.uk',
  'dailysceptic.org',
  'dailytelegraph.com.au',
  'dailywire.com',
  'dallasnews.com',
  'dasinvestment.com',
  'daytondailynews.com',
  'decanter.com',
  'defector.com',
  'delish.com',
  'democratandchronicle.com',
  'demorgen.be',
  'denik.cz',
  'denverpost.com',
  'derbund.ch',
  'derpatriot.de',
  'derstandard.at',
  'desmoinesregister.com',
  'destentor.nl',
  'detroitnews.com',
  'df.cl',
  'dhnet.be',
  'di.se',
  'diabeteshealth.co.in',
  'diariocorreo.pe',
  'diariodemallorca.es',
  'diariodenavarra.es',
  'diariodonordeste.verdesmares.com.br',
  'diariosur.es',
  'diariovasco.com',
  'die-tagespost.de',
  'diepresse.com',
  'digiday.com',
  'digiday.jp',
  'diplomatique.org.br',
  'discovermagazine.com',
  'dispatch.com',
  'dk-online.de',
  'dn.no',
  'dn.pt',
  'dn.se',
  'dna.fr',
  'dnevnik.bg',
  'document.no',
  'doorbraak.be',
  'dwell.com',
  'eastbaytimes.com',
  'eastwest.eu',
  'echo-online.de',
  'economictimes.com',
  'economictimes.indiatimes.com',
  'economist.com',
  'ecran-total.fr',
  'ed.nl',
  'editorialedomani.it',
  'edweek.org',
  'ekuriren.se',
  'elcomercio.es',
  'elcomercio.pe',
  'elconfidencial.com',
  'elcorreo.com',
  'eldeber.com.bo',
  'eldia.es',
  'eldiario.es',
  'eldiarioar.com',
  'eldiariomontanes.es',
  'elespanol.com',
  'elespectador.com',
  'elfinancierocr.com',
  'elle.com',
  'elle.fr',
  'elledecor.com',
  'elmercurio.com',
  'elmundo.es',
  'elnortedecastilla.es',
  'elnuevoherald.com',
  'elobservador.com.uy',
  'elordenmundial.com',
  'elpais.com',
  'elpais.com.uy',
  'elperiodico.com',
  'eltiempo.com',
  'eltribuno.com',
  'eluniversal.com.mx',
  'eluniverso.com',
  'em.com.br',
  'endpoints.news',
  'energeia.nl',
  'engineersireland.ie',
  'enotes.com',
  'ensighten.com',
  'entrepreneur.com',
  'epaper.indiatimes.com',
  'epe.es',
  'epicurious.com',
  'epoch.org.il',
  'espn.com',
  'esprit.presse.fr',
  'esquire.com',
  'estadao.com.br',
  'estrepublicain.fr',
  'euobserver.com',
  'euractiv.com',
  'euroinvestor.dk',
  'european-rubber-journal.com',
  'europower.no',
  'eurotransport.de',
  'eviemagazine.com',
  'evolok.net',
  'exame.com',
  'examiner.com.au',
  'examtopics.com',
  'expansion.com',
  'express.co.uk',
  'expressnews.com',
  'expresso.pt',
  'falter.at',
  'farmersjournal.ie',
  'farodevigo.es',
  'fastcompany.com',
  'faz.net',
  'femmesdaujourdhui.be',
  'fewcents.co',
  'fieldandstream.com',
  'finance.si',
  'financialexpress.com',
  'financialpost.com',
  'firstthings.com',
  'fiskeribladet.no',
  'flair.be',
  'flair.nl',
  'flamman.se',
  'flugrevue.de',
  'flz.de',
  'fmrmagazine.com',
  'fnarena.com',
  'fnlondon.com',
  'focusplus.de',
  'fokus.se',
  'football-italia.net',
  'forbes.com',
  'forbes.com.au',
  'forbes.pl',
  'forbes.sk',
  'forbes.ua',
  'foreignaffairs.com',
  'foreignpolicy.com',
  'fortune.com',
  'foxnews.com',
  'fr.de',
  'fraenkischertag.de',
  'franc-tireur.fr',
  'freep.com',
  'freiepresse.de',
  'freitag.de',
  'fresnobee.com',
  'frieze.com',
  'ft.com',
  'ftm.eu',
  'ftm.nl',
  'ga.de',
  'gauchazh.clicrbs.com.br',
  'gazetadopovo.com.br',
  'gazzetta.it',
  'gbnews.com',
  'gea.de',
  'geelongadvertiser.com.au',
  'gelderlander.nl',
  'genomeweb.com',
  'gestion.pe',
  'glassdoor.com',
  'globes.co.il',
  'globo.com',
  'glossy.co',
  'gocomics.com',
  'goldcoastbulletin.com.au',
  'golem.de',
  'golf.com',
  'goodhousekeeping.com',
  'gothamist.com',
  'gp.se',
  'gplanet.co.il',
  'gq.com',
  'gramophone.co.uk',
  'granta.com',
  'groene.nl',
  'ground.news',
  'grubstreet.com',
  'gulfnews.com',
  'gutepillen-schlechtepillen.de',
  'gva.be',
  'haaretz.co.il',
  'haaretz.com',
  'haller-kreisblatt.de',
  'handelsblatt.com',
  'handelszeitung.ch',
  'harpers.org',
  'harpersbazaar.com',
  'hartenergy.com',
  'hartfordbusiness.com',
  'harzkurier.de',
  'haz.de',
  'hbr.org',
  'hbrtaiwan.com',
  'hbvl.be',
  'hcn.org',
  'hd.se',
  'heatmap.news',
  'heidi.news',
  'heise.de',
  'heraldsun.com.au',
  'hilltimes.com',
  'himalmag.com',
  'hindustantimes.com',
  'historia.fr',
  'historischnieuwsblad.nl',
  'historyextra.com',
  'historytoday.com',
  'hln.be',
  'hochparterre.ch',
  'hola.com',
  'housebeautiful.com',
  'houstonchronicle.com',
  'hoy.es',
  'hpdetijd.nl',
  'hsfo.dk',
  'hsj.co.uk',
  'huisartsvandaag.nl',
  'humanite.fr',
  'humo.be',
  'hydrogeninsight.com',
  'iai.tv',
  'ideal.es',
  'idowa.de',
  'if.com.au',
  'iflscience.com',
  'ikz-online.de',
  'ilfattoquotidiano.it',
  'ilfoglio.it',
  'ilgazzettino.it',
  'ilgiorno.it',
  'ilglobo.com',
  'illawarramercury.com.au',
  'ilmanifesto.it',
  'ilmattino.it',
  'ilmessaggero.it',
  'ilrestodelcarlino.it',
  'ilsecoloxix.it',
  'ilsole24ore.com',
  'iltelegrafolivorno.it',
  'iltirreno.it',
  'impactalpha.com',
  'inc.com',
  'inc42.com',
  'independent.co.uk',
  'indianexpress.com',
  'indiatoday.in',
  'indystar.com',
  'inews.co.uk',
  'infolibre.es',
  'informacion.es',
  'infzm.com',
  'inkl.com',
  'inquirer.com',
  'inscript.me',
  'insidehighered.com',
  'insidermonkey.com',
  'insights.citeline.com',
  'intelligentinvestor.com.au',
  'interestingengineering.com',
  'internationalepolitik.de',
  'internazionale.it',
  'intrafish.com',
  'intrafish.no',
  'investing.com',
  'investmentnews.com',
  'investors.com',
  'investorschronicle.co.uk',
  'ipe.com',
  'ipolitics.ca',
  'iraqoilreport.com',
  'irishexaminer.com',
  'irishnews.com',
  'irishtimes.com',
  'italian.tech',
  'italiaoggi.it',
  'jacksonville.com',
  'jacobin.de',
  'japantimes.co.jp',
  'jazzwise.com',
  'jerseyeveningpost.com',
  'jeuneafrique.com',
  'jewishinsider.com',
  'jgnt.co',
  'joc.com',
  'jornaldocomercio.com',
  'journaldemillau.fr',
  'journaldemontreal.com',
  'journaldequebec.com',
  'journaldunet.com',
  'journalnow.com',
  'journalstar.com',
  'jpost.com',
  'jsonline.com',
  'kalkinemedia.com',
  'kansas.com',
  'kansascity.com',
  'kathimerini.gr',
  'kentucky.com',
  'keymilitary.com',
  'kinotico.es',
  'kleinezeitung.at',
  'kn-online.de',
  'knack.be',
  'knoxnews.com',
  'kommersant.ru',
  'kompas.id',
  'komputerswiat.pl',
  'krautreporter.de',
  'kristeligt-dagblad.dk',
  'ksta.de',
  'ktipp.ch',
  'kurier.at',
  'kw.be',
  'kystens.no',
  'la-croix.com',
  'labusinessjournal.com',
  'lacote.ch',
  'ladepeche.fr',
  'ladiaria.com.uy',
  'lagaceta.com.ar',
  'lalibre.be',
  'lalsace.fr',
  'lamanchelibre.fr',
  'lamontagne.fr',
  'lanacion.com.ar',
  'lanazione.it',
  'lance.com.br',
  'lanuovasardegna.it',
  'lapost.com',
  'lapresse.ca',
  'laprovincia.es',
  'lardennais.fr',
  'larepubliquedespyrenees.fr',
  'largus.fr',
  'larioja.com',
  'lasegunda.com',
  'lasprovincias.es',
  'lastampa.it',
  'lasvegasadvisor.com',
  'lataco.com',
  'latercera.com',
  'latimes.com',
  'lavanguardia.com',
  'lavenir.net',
  'laverdad.es',
  'lavialibera.it',
  'lavoixdunord.fr',
  'lavoz.com.ar',
  'lavozdegalicia.es',
  'lavozdigital.es',
  'law.com',
  'le1hebdo.fr',
  'lecanardenchaine.fr',
  'lecho.be',
  'lecourrierdesstrateges.fr',
  'ledauphine.com',
  'ledevoir.com',
  'lefigaro.fr',
  'legalbites.in',
  'legrandcontinent.eu',
  'lehighvalleylive.com',
  'lejdd.fr',
  'lejsl.com',
  'lemagit.fr',
  'lemessager.fr',
  'lemonde.fr',
  'lenouveleconomiste.fr',
  'lenouvelliste.ch',
  'leparisien.fr',
  'lepoint.fr',
  'leprogres.fr',
  'lequipe.fr',
  'lequotidiendumedecin.fr',
  'lequotidiendupharmacien.fr',
  'lerevenu.com',
  'lesaffaires.com',
  'lescienze.it',
  'lesechos.fr',
  'lesinrocks.com',
  'lesoir.be',
  'lesoleil.com',
  'lest-eclair.fr',
  'letelegramme.fr',
  'letemps.ch',
  'levante-emv.com',
  'levif.be',
  'lexpress.fr',
  'libelle.be',
  'libelle.nl',
  'liberation-champagne.fr',
  'liberation.fr',
  'liga.net',
  'linda.nl',
  'lindependant.fr',
  'linforme.com',
  'literaryreview.co.uk',
  'livelaw.in',
  'livemint.com',
  'lkz.de',
  'ln-online.de',
  'lne.es',
  'loebclassics.com',
  'loeildelaphotographie.com',
  'lopinion.fr',
  'losandes.com.ar',
  'lrb.co.uk',
  'luatkhoa.net',
  'lunion.fr',
  'luxurydaily.com',
  'lvz.de',
  'lz.de',
  'macrobusiness.com.au',
  'madison.com',
  'madraspaper.com',
  'magazyn-kuchnia.pl',
  'main-echo.de',
  'mainpost.de',
  'makorrishon.co.il',
  'mallorcazeitung.es',
  'mamamia.com.au',
  'manilatimes.net',
  'manoramaonline.com',
  'marca.com',
  'margriet.nl',
  'marketnews.com',
  'marketscreener.com',
  'marketwatch.com',
  'masslive.com',
  'matheranalytics.com',
  'maz-online.de',
  'mcall.com',
  'mcclatchydc.com',
  'mediapart.fr',
  'medieninsider.com',
  'medium.com',
  'medpagetoday.com',
  'medscape.com',
  'memberstack.com',
  'menshealth.com',
  'mercurynews.com',
  'meretmarine.com',
  'merkur.de',
  'mexiconewsdaily.com',
  'miamiherald.com',
  'mid-day.com',
  'midilibre.fr',
  'milanofinanza.it',
  'milenio.com',
  'milesplit.com',
  'military.com',
  'mims.com',
  'mining-journal.com',
  'mittelbayerische.de',
  'mlive.com',
  'moda.it',
  'modernhealthcare.com',
  'modernrailways.com',
  'modernretail.co',
  'monacomatin.mc',
  'monocle.com',
  'morgenpost.de',
  'motorradonline.de',
  'motorsportmagazine.com',
  'moustique.be',
  'mt.de',
  'mundodeportivo.com',
  'museumsassociation.org',
  'musicomh.com',
  'mv-voice.com',
  'myanmar-now.org',
  'mz.de',
  'nachrichten.at',
  'nacion.com',
  'narcity.com',
  'natgeomedia.com',
  'nation.africa',
  'nationalgeographic.com',
  'nationalgeographic.com.es',
  'nationalgeographic.it',
  'nationalgeographic.nl',
  'nationalobserver.com',
  'nationalpost.com',
  'nationalreview.com',
  'naturalgasworld.com',
  'nature.com',
  'nautil.us',
  'nbcnews.com',
  'ndtvprofit.com',
  'neuepresse.de',
  'newcastleherald.com.au',
  'newcriterion.com',
  'newleftreview.org',
  'newoxfordreview.org',
  'newrepublic.com',
  'news-press.com',
  'newsblues.com',
  'newscientist.com',
  'newsday.com',
  'newslaundry.com',
  'newsmemory.com',
  'newsobserver.com',
  'newspicks.com',
  'newstatesman.com',
  'newsweek.com',
  'newsweek.pl',
  'newyorker.com',
  'nhregister.com',
  'niagarafallsreview.ca',
  'nicematin.com',
  'nieuwsblad.be',
  'nj.com',
  'nn.de',
  'nola.com',
  'nordjyske.dk',
  'nordlittoral.fr',
  'nordsee-zeitung.de',
  'northerndailyleader.com.au',
  'northjersey.com',
  'nouvelobs.com',
  'novethic.fr',
  'noz.de',
  'nrc.nl',
  'nrpyrenees.fr',
  'nrz.de',
  'nsctotal.com.br',
  'ntnews.com.au',
  'ntz.de',
  'nu.nl',
  'nv.ua',
  'nw.de',
  'nwitimes.com',
  'nwzonline.de',
  'nybooks.com',
  'nydailynews.com',
  'nymag.com',
  'nypost.com',
  'nypost.djservices.io',
  'nysun.com',
  'nyteknik.se',
  'nytimes.com',
  'nzgeo.com',
  'nzherald.co.nz',
  'nzz.ch',
  'observador.pt',
  'observer.co.uk',
  'ocbj.com',
  'ocregister.com',
  'odt.co.nz',
  'ogj.com',
  'oklahoman.com',
  'oko.press',
  'ole.com.ar',
  'omaha.com',
  'omeda.com',
  'on3.com',
  'onecount.net',
  'onet.pl',
  'opopular.com.br',
  'opovo.com.br',
  'oprahdaily.com',
  'orangebloods.com',
  'oregonlive.com',
  'orlandosentinel.com',
  'ostsee-zeitung.de',
  'otz.de',
  'ouest-france.fr',
  'outdoorlife.com',
  'outlookbusiness.com',
  'outlookindia.com',
  'outsideonline.com',
  'ovb-online.de',
  'overwaar.de',
  'oxygenmag.com',
  'palmbeachpost.com',
  'paloaltoonline.com',
  'paris-normandie.fr',
  'parismatch.com',
  'parkiet.com',
  'parool.nl',
  'pastemagazine.com',
  'pb.pl',
  'pbn.com',
  'pebmed.com.br',
  'pelcro.com',
  'pennlive.com',
  'perspectivemedia.com',
  'petitbleu.fr',
  'pharmaceutical-journal.com',
  'philanthropy.com',
  'phillymag.com',
  'philomag.com',
  'philomag.de',
  'philonomist.com',
  'philosophynow.org',
  'physicsworld.com',
  'piano.io',
  'pico.tools',
  'pilotonline.com',
  'pionline.com',
  'pitchfork.com',
  'plasticsnews.com',
  'ploetzblog.de',
  'pmg.be',
  'pnp.de',
  'pokerindustrypro.com',
  'policinginsight.com',
  'politicaexterior.com',
  'politico.com',
  'politiken.dk',
  'polityka.pl',
  'poool.fr',
  'popsci.com',
  'popularmechanics.com',
  'post-gazette.com',
  'postandcourier.com',
  'pourlascience.fr',
  'pourleco.com',
  'precisionmedicineonline.com',
  'prensa.com',
  'prephoops.com',
  'pressenterprise.com',
  'pressgazette.co.uk',
  'prevention.com',
  'profi.de',
  'profil.at',
  'project-syndicate.org',
  'prospectmagazine.co.uk',
  'public.fr',
  'publico.es',
  'publishersweekly.com',
  'puck.news',
  'punters.com.au',
  'pv-digest.de',
  'pzc.nl',
  'qcintel.com',
  'qiota.com',
  'qpbriefing.com',
  'quadrant.org.au',
  'quest.nl',
  'quickmath.com',
  'quora.com',
  'quotidiano.net',
  'quotidianodipuglia.it',
  'qz.com',
  'racketmn.com',
  'racquetmag.com',
  'railwaygazette.com',
  'rawstory.com',
  'rd.nl',
  'rechargenews.com',
  'reforme.net',
  'repubblica.it',
  'republicain-lorrain.fr',
  'researchprofessionalnews.com',
  'reuters.com',
  'reviewjournal.com',
  'revistaoeste.com',
  'revue21.fr',
  'revueconflits.com',
  'rheinpfalz.de',
  'richmond.com',
  'riotimesonline.com',
  'rnd.de',
  'roadandtrack.com',
  'rockdelux.com',
  'rollingstone.com',
  'rotowire.com',
  'rp-online.de',
  'rp.pl',
  'rtoinsider.com',
  'rubbernews.com',
  'rugbypass.com',
  'rugbyrama.fr',
  'rundschau-online.de',
  'runnersworld.com',
  'saarbruecker-zeitung.de',
  'sabado.pt',
  'sacbee.com',
  'saechsische.de',
  'sandiegouniontribune.com',
  'santa.lv',
  'scholastic.com',
  'schwarzwaelder-bote.de',
  'schweizermonat.ch',
  'science-et-vie.com',
  'science.org',
  'sciencenews.org',
  'sciencesetavenir.fr',
  'scientificamerican.com',
  'scmp.com',
  'scotsman.com',
  'screendaily.com',
  'scroll.in',
  'sdbj.com',
  'seafoodsource.com',
  'seattletimes.com',
  'semana.com',
  'serenitymarkets.com',
  'sfchronicle.com',
  'sfstandard.com',
  'sfvbj.com',
  'shreveportbossieradvocate.com',
  'shrm.org',
  'shz.de',
  'si.com',
  'sifted.eu',
  'siliconrepublic.com',
  'silive.com',
  'sinarharian.com.my',
  'skepticalinquirer.org',
  'skimag.com',
  'sky.it',
  'slate.com',
  'slideshare.net',
  'sloanreview.mit.edu',
  'smartcompany.com.au',
  'smh.com.au',
  'sn.at',
  'sofrep.com',
  'solarserver.de',
  'solicitorsjournal.com',
  'sophi.io',
  'sourcingjournal.com',
  'southernstar.ie',
  'southwarknews.co.uk',
  'spacenews.com',
  'speakup.it',
  'spectator.com',
  'spectator.com.au',
  'spectator.org',
  'spectrejournal.com',
  'spektrum.de',
  'speld.nl',
  'spglobal.com',
  'spiegel.de',
  'spiked-online.com',
  'splainer.in',
  'splinter.com',
  'spokesman.com',
  'sportico.com',
  'sportsbusinessjournal.com',
  'springermedizin.de',
  'stabmag.com',
  'standaard.be',
  'standard.co.uk',
  'standard.net.au',
  'standardmedia.co.ke',
  'standcolumbia.org',
  'star-telegram.com',
  'staradvertiser.com',
  'startribune.com',
  'statesman.com',
  'statnews.com',
  'stcatharinesstandard.ca',
  'steadyhq.com',
  'stereogum.com',
  'stern.de',
  'stimme.de',
  'stltoday.com',
  'stocknews.com',
  'straitstimes.com',
  'strategies.fr',
  'stratfor.com',
  'stratford-herald.com',
  'stripes.com',
  'study.com',
  'stuttgarter-nachrichten.de',
  'stuttgarter-zeitung.de',
  'stylist.co.uk',
  'subscriptioninsider.com',
  'sudinfo.be',
  'sudouest.fr',
  'sueddeutsche.de',
  'suedkurier.de',
  'sun-sentinel.com',
  'suomensotilas.fi',
  'supchina.com',
  'superdeporte.es',
  'sustainableviews.com',
  'swarajyamag.com',
  'sydsvenskan.se',
  'syracuse.com',
  'table.media',
  'tagesanzeiger.ch',
  'tagesspiegel.de',
  'talkingpointsmemo.com',
  'tampabay.com',
  'taxscan.in',
  'tdg.ch',
  'teachingprofessor.com',
  'teachingtimes.com',
  'techinasia.com',
  'technologyreview.com',
  'techtarget.com',
  'telecompaper.com',
  'telegraaf.nl',
  'telegraph.co.uk',
  'telerama.fr',
  'tennessean.com',
  'tes.com',
  'tetu.com',
  'texasmonthly.com',
  'the-american-interest.com',
  'the-fence.com',
  'the-independent.com',
  'the-past.com',
  'the-scientist.com',
  'the-star.co.ke',
  'the-tls.com',
  'the42.ie',
  'theadvocate.com',
  'theadvocate.com.au',
  'theage.com.au',
  'theamericanconservative.com',
  'theamericanscholar.org',
  'theartnewspaper.com',
  'theatlantic.com',
  'theaustralian.com.au',
  'thebaffler.com',
  'thebanker.com',
  'thebanner.com',
  'theblaze.com',
  'thebookseller.com',
  'thebulletin.org',
  'thecaterer.com',
  'thecatholicherald.com',
  'thechronicle.com.au',
  'thecore.in',
  'thecourier.com.au',
  'thecritic.co.uk',
  'thecut.com',
  'thedailybeast.com',
  'thedailymash.co.uk',
  'theday.com',
  'thediplomat.com',
  'thedispatch.com',
  'thedriftmag.com',
  'theepochtimes.com',
  'thefashionlaw.com',
  'thefederal.com',
  'theglobeandmail.com',
  'theheadline.jp',
  'thehill.com',
  'thehindu.com',
  'thehindubusinessline.com',
  'thehub.ca',
  'theijf.org',
  'theintercept.com',
  'theirishfield.ie',
  'thejc.com',
  'thejuggernaut.com',
  'thelampmagazine.com',
  'thelawyer.com',
  'theleaflet.in',
  'theloadstar.com',
  'thelogic.co',
  'themandarin.com.au',
  'themarker.com',
  'themarket.ch',
  'themercury.com.au',
  'themonthly.com.au',
  'thenation.com',
  'thenationalpulse.com',
  'thenewatlantis.com',
  'thenewslens.com',
  'thenewsminute.com',
  'thenewworld.co.uk',
  'thepeterboroughexaminer.com',
  'thepointmag.com',
  'theprovince.com',
  'theqt.online',
  'thequint.com',
  'therealdeal.com',
  'therecord.com',
  'thesaturdaypaper.com.au',
  'theschooloflife.com',
  'thescottishsun.co.uk',
  'thespec.com',
  'thestage.co.uk',
  'thestar.com',
  'thestate.com',
  'thesun.co.uk',
  'thesun.ie',
  'thetablet.co.uk',
  'thetablet.org',
  'thetimes.com',
  'theurbandeveloper.com',
  'theverge.com',
  'theweek.com',
  'thewest.com.au',
  'thewrap.com',
  'thisismoney.co.uk',
  'thueringer-allgemeine.de',
  'tidningenridsport.se',
  'tijd.be',
  'tijorifinance.com',
  'timeshighereducation.com',
  'timeslive.co.za',
  'timesunion.com',
  'tirebusiness.com',
  'tlz.de',
  'tokyo-np.co.jp',
  'toledoblade.com',
  'tomshardware.com',
  'topagrar.com',
  'torontosun.com',
  'totalfootballanalysis.com',
  'towardsdatascience.com',
  'townandcountrymag.com',
  'townsvillebulletin.com.au',
  'tradewindsnews.com',
  'trailrunnermag.com',
  'tri-cityherald.com',
  'triathlete.com',
  'tribunedelyon.fr',
  'tribuneindia.com',
  'trouw.nl',
  'tubantia.nl',
  'tucson.com',
  'tulsaworld.com',
  'tuttosport.com',
  'tvnewscheck.com',
  'twincities.com',
  'unherd.com',
  'united.no',
  'uol.com.br',
  'upstreamonline.com',
  'usatoday.com',
  'usinenouvelle.com',
  'utech-polyurethane.com',
  'utilityweek.co.uk',
  'valeursactuelles.com',
  'valueresearchonline.com',
  'vancouversun.com',
  'vanityfair.com',
  'variety.com',
  'varmatin.com',
  'vegetariantimes.com',
  'velikeprice.com',
  'vibilagare.se',
  'vice.com',
  'vikatan.com',
  'ville-rail-transports.com',
  'visegradinsight.eu',
  'vn.at',
  'vn.nl',
  'vogue.co.uk',
  'vogue.com',
  'voguebusiness.com',
  'vol.at',
  'volksfreund.de',
  'volkskrant.nl',
  'volksstimme.de',
  'vosgesmatin.fr',
  'vox.com',
  'vsin.com',
  'vulture.com',
  'warontherocks.com',
  'washingtonexaminer.com',
  'washingtonpost.com',
  'washingtontimes.com',
  'watoday.com.au',
  'waz.de',
  'wbez.org',
  'wbjournal.com',
  'weekendavisen.dk',
  'weeklytimesnow.com.au',
  'wellandtribune.ca',
  'welt.de',
  'weltkunst.de',
  'weser-kurier.de',
  'westcoastpreps.com',
  'westernadvocate.com.au',
  'westernjournal.com',
  'westernstandard.news',
  'wiesbadener-kurier.de',
  'winnipegfreepress.com',
  'wired.com',
  'wissenschaft.de',
  'wiwo.de',
  'wochenblatt.com',
  'womenshealth.de',
  'womenshealthmag.com',
  'womensrunning.com',
  'wonderzine.com',
  'worldeconomics.com',
  'wp.de',
  'wr.de',
  'wsj.com',
  'wwd.com',
  'wyborcza.biz',
  'wyborcza.pl',
  'wysokieobcasy.pl',
  'xiaohu.ai',
  'ynet.co.il',
  'yogajournal.com',
  'yorkshirepost.co.uk',
  'zeit.de',
  'zeitzeichen.net',
  'zephr.com',
};
