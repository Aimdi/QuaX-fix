...
Color stockChangeColour(bool? isUp) => (isUp ?? true) ? kStockUpColour : kStockDownColour;

final NumberFormat stockMoneyFormat = NumberFormat.decimalPatternDigits(decimalDigits: 2);

String stockPercentLabel(double percent) => '${percent >= 0 ? '+' : ''}${percent.toStringAsFixed(2)}%';

String stockChangeLabel(double change) => '${change >= 0 ? '+' : ''}${stockMoneyFormat.format(change)}';
